...
import {
  ...
  industryTypeConfig,
  isoConfig,
  isPrototypeMoldingMachineConfig,
  moldingEquipmentConfig,
  resinUsedConfig,
  searchConfig,
  searchTypeConfig,
  subsidyConfig,
...
const Search = ({
  ...
  const startSearch = async () => {
    ...
    searchObj = { ...searchObj, searchType: searchType };
            ...
            <div className="w-full">
              <div className="grid grid-cols-8 gap-2 rounded w-full">
                ...
                <ul className="w-full col-span-8 border p-2 rounded">
                  {moldingEquipmentConfig.map((mec: string, index: number) => {
              ...
              <div className="grid grid-cols-8 gap-2 rounded w-full">
                ...
                <ul className="w-full col-span-8 border p-2 rounded">
                  {clampingForceConfig.map((cfc: string, index: number) => {
                    const selectWeight = () => {
                      if (
                        clampingForceConfigRefs.current[index].current?.checked
                      ) {
                ...
                <ul className="w-full col-span-8 border p-2 rounded">
                  {resinUsedConfig.map((ruc: string, index: number) => {
                    const selectResinUsedType = () => {
                      if (resinUsedConfigRefs.current[index].current?.checked) {
                        setResin([...resin, ruc]);
                      } else {
                ...
                <ul className="w-full col-span-8 border p-2 rounded">
                  {cleanRoomConfig.map((crc: string, index: number) => {
                    const selectCleanRoom = () => {
                      if (cleanRoomConfigRefs.current[index].current?.checked) {
    ...
    <div>
      <div className="border border-gray-300 p-3 grid grid-cols-1 gap-3 bg-white shadow-lg rounded-lg h-full">
        ...
        <div className="grid grid-cols-8 gap-2 rounded w-full">
          <select
            className="col-span-8 border p-2 rounded"
            onChange={(e) => setSerachType(e.target.value)}
          >
            {searchTypeConfig.map((stc, index) => (
              <option key={index}>{stc}</option>
            ))}
          </select>
        </div>
